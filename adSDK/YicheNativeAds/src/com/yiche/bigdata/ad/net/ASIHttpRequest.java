...
import com.yiche.bigdata.ad.json.JSONHelper;
import com.yiche.bigdata.ad.net.NetMessage.NetMessageType;
import com.yiche.bigdata.ad.netresponse.BaseResult;
import com.yiche.bigdata.ad.utils.FileHelper;
...
public class ASIHttpRequest implements Runnable
{
    ...
    private int mTimeOutSeconds = 10 * 1000;
    ...
    @SuppressLint("DefaultLocale") private void handleNetRequest()
    {
            ...
            do
            {
                ...
                if (responseCode == HttpStatus.SC_OK || responseCode == HttpStatus.SC_PARTIAL_CONTENT)
                {
                    ...
                    if (responseDataLen != -1)
                    {
                        ...
                        {
                            handleErrorEvent("网络异常，请检查网络状态后重试！", YCErrorCode.YC_NET_DATA_ERROR);
                            break;
                        }
                    ...
                    else
                    {
                        ...
                        {
                            // When download file
                            handleErrorEvent("content len is error", YCErrorCode.YC_NET_GENER_ERROR);
                        }
    ...
    private void handleNetGetRequest()
    {
    	try {
            ...
            if (code == 200) {
            	String string ="{\"status\":2000}";
            	handleSuccessEvent(string.getBytes());
            } 
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//					getRequest
//							.setHeader("RANGE", "bytes=" + tempfileSize + "-");
//				}
//				try {
//					response = client.execute(getRequest);
//				} catch (Exception e) {
